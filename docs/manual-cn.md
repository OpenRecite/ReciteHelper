...
可以配置课程号、考试时长、题目数量以及各章节抽题权重。权重总和不必手动调整为 100%，软件会自动换算比例。分值固定为：选择题 3 分、填空题每空 1 分、判断题 1 分、名词解释 4 分、解答题 5 分。

功能菜单中的“导入试卷”支持 PDF/TXT。一个文件可以包含多套试卷，DeepSeek 会按套分割并生成答案与解析，结果保存在项目的 `exams` 目录。进入模拟考试后启用“加载套卷”，即可选择原卷；此时默认组卷参数会被禁用。套卷 JSON 中的 `small_title` 和 `main_title` 可分别自定义试卷小标题与大标题。
