...
ReciteHelper 是一款面向考试复习、课程学习与知识整理的 AI 桌面应用。它可以将学习资料转换为带有章节、知识点、选择题、简答题和本地知识库的 `.rhproj` 学习项目，并提供章节练习、智能复习、模拟考试和错题解析等功能。
...
- `RStandard`：简答题判定使用的相似度阈值，通常不需要修改。
...
2. **知识提取**：分块调用 AI，生成知识点、单项选择题和简答题。
...
- **简答题**：输入自己的答案后提交。系统会综合文本相似度进行判定，不要求与参考答案逐字一致。
...
可以配置课程号、考试时长、题目数量、每题分数以及各章节抽题权重。权重总和不必手动调整为 100%，软件会自动换算比例。
...
在章节选择窗口打开功能菜单并选择导出，软件会在项目目录中生成 `rh_output.zip`。导出包包含项目文件、清单以及可用的知识库文件，适合备份或发送给其他用户。
